...
### POC 운영 체크리스트
- 수집 실패 로그를 배치 실행 단위로 누락 없이 기록했는가? (`timestamp`, 대상 URL, 실패 단계, 에러 요약)
- `UNKNOWN` 비율을 회차별로 집계하고 임계치(`10%`) 초과 여부를 확인했는가?
- 수동 검토 소요시간을 회차별로 측정하고 평균 `30분/50건` 이내 여부를 확인했는가?
- 5회 연속 acceptance test 통과 여부를 확인했는가?

### Ralph 루프 운영 규칙
- **Step A (기준 측정)**: acceptance tests를 실행해 현재 회차의 기준값을 측정합니다.
- **Step B (단일 병목 개선)**: 미달 지표 중 **1개만 선택**해 개선 작업을 수행합니다.
- **Step C (재검증)**: `lint/typecheck/build/test`를 재실행해 회귀 여부를 확인합니다.
- **Step D (반복)**: 아래 종료 조건을 충족하기 전까지 Step A~C를 반복합니다.

### Ralph 루프 종료 조건(정량 기준)
- 문서 기준 1: 수집 실패 로그가 배치 실행 단위로 누락 없이 기록됨 (`timestamp`, 대상 URL, 실패 단계, 에러 요약).
- 문서 기준 2: `UNKNOWN` 비율이 임계치 `10%` 이하로 유지됨.
- 문서 기준 3: 수동 검토 소요시간 평균이 `30분/50건` 이내임.
- 품질 게이트: acceptance tests가 **5회 연속 PASS**임.
- 최종 종료는 위 **문서 기준 3개 모두 충족 + 5회 연속 PASS**를 동시에 만족할 때로 정의합니다.
