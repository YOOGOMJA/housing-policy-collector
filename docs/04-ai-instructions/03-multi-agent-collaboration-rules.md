# 멀티 에이전트 협업 규칙 (PM/PO 운영안)

## 1) 현재 세팅 기준 진단
현재 문서 체계는 **순차 실행 정의**에는 강점이 있지만, 다음 항목이 비어 있습니다.

1. 병렬 작업 시 충돌 방지 규칙(소유권, 락, 우선순위)
2. 에이전트 간 인터페이스 계약(입출력 스키마, 상태 전이)
3. 품질 게이트의 실행 책임(누가 언제 무엇을 검증하는지)
4. 작업 인수인계 규칙(완료 정의, 증적, 롤백 기준)
5. 의사결정 이력(ADR) 및 변경 영향도 추적

## 2) 프로젝트 특성 기반 필수 추가 규칙
본 프로젝트는 정책 정확도/개인정보 보호/알림 신뢰성이 핵심이므로, 아래를 필수 규칙으로 둡니다.

### A. 작업 분할과 소유권
- 각 작업은 `한 에이전트 1 오너` 원칙으로 수행합니다.
- 공용 산출물은 섹션 단위 소유권을 명시합니다.
  - 예: `02-data-model.md`의 개인정보 필드는 Data-Agent 소유, 정책 조건은 Policy-Agent 승인 필수
- 동일 파일 동시 수정이 예상되면 선행으로 변경 범위를 합의합니다.

### B. 병렬 작업 제어
- 병렬 가능한 작업과 선행 의존 작업을 분리합니다.
  - 병렬 가능: Collector-Agent(수집 실패 감지) ↔ Notifier-Agent(전송/중복방지)
  - 의존 필요: Policy-Agent 정책 확정 → Matcher-Agent 판정 로직
- 병렬 작업 시 공통 계약 문서를 먼저 고정합니다.
  - 데이터 모델 버전
  - 이벤트/알림 페이로드 스키마
  - 상태 코드(성공/검토필요/실패)

### C. 품질 게이트(DoD)
- 코드 변경이 있는 경우:
  1. 린트 통과
  2. 컨벤션 체크 통과(커밋/네이밍/포맷)
- 문서 변경이 있는 경우:
  1. YAML 문서일 때만 포맷/문법 체크 실행
  2. 신규/변경 문서가 문서 인덱스 및 연관 문서에 반영됐는지 확인
- 정책 관련 변경이 있는 경우:
  - `검토필요` 분기 조건 및 개인정보 처리 영향도를 체크리스트로 첨부

### D. 인수인계 규칙
- 에이전트 완료 보고는 아래 4가지를 포함합니다.
  1. 변경 파일 목록
  2. 결정 사항과 근거
  3. 미해결 리스크
  4. 다음 에이전트 액션 아이템
- 다음 단계 에이전트는 이전 단계 산출물의 버전 태그를 참조해 시작합니다.

### E. 운영/장애 관점 협업
- Collector-Agent와 Ops-Agent는 수집 실패 이벤트 코드 체계를 공유합니다.
- Notifier-Agent와 Ops-Agent는 알림 지연/중복 지표 정의를 공유합니다.
- Incident 대응 시 PM/PO 판단이 필요한 정책 변경은 즉시 Product-Agent+Policy-Agent 동시 검토로 승격합니다.

## 3) 권장 산출물(추가하면 좋은 것)
1. `docs/04-ai-instructions/agent-ownership-matrix.yaml`
   - 에이전트별 책임 문서/승인권/대체 담당자
2. `docs/04-ai-instructions/interface-contracts.yaml`
   - 에이전트 간 입출력 스키마와 버전
3. `docs/03-operations/change-impact-checklist.yaml`
   - 정책/데이터/알림 변경 시 영향도 점검 항목
4. `docs/04-ai-instructions/decision-log.md`
   - 충돌 해결 및 예외 승인 이력(ADR 요약)

## 4) 즉시 적용 우선순위 (PM/PO 권고)
1. 소유권 매트릭스 + 인터페이스 계약부터 고정
2. 병렬 작업 시작 전 공통 상태코드/스키마 동결
3. PR 단위로 DoD 체크리스트 의무화
4. 운영 지표(지연/중복/실패)와 장애 코드를 에이전트 공통 언어로 통일


## 5) Git worktree 운영 규칙 (동시 작업 필수)
- 병렬 구현은 브랜치만 분리하지 말고 worktree까지 분리해 파일 충돌/오염을 줄입니다.
- 표준 절차:
  1. 메인 작업 디렉터리에서 기준 브랜치 최신화
  2. `git worktree add ../wt-<agent> -b <branch> <base-branch>`로 에이전트별 작업 트리 생성
  3. 각 에이전트는 자기 worktree에서만 커밋
  4. 완료 후 통합 브랜치에서 순차 병합 및 회귀 점검
  5. 종료 시 `git worktree remove ../wt-<agent>`로 정리
- 운영 안전장치:
  - worktree별 `CODEOWNER/소유 문서`를 명시
  - 공통 계약 파일 변경은 단일 전담 브랜치에서만 수행
  - 통합 전 `git worktree list`로 잔여 작업 트리 점검

### 현재 환경 적용 가능 여부
- 본 저장소/실행 환경에서 `git worktree` 명령 사용이 가능합니다.
- 따라서 서브에이전트 병렬 작업은 **브랜치 분리 + worktree 분리**를 기본 정책으로 채택하는 것이 적합합니다.


## 6) 문서/PR 언어 규칙
- 저장소 내 운영/정책/기술 문서는 한국어로 작성합니다(고유명사, 코드 식별자, 표준 스펙 명칭은 예외).
- PR 제목/본문/체크리스트/리뷰 응답은 한국어를 기본으로 작성합니다.
- 기존 영문 중심 PR 설명이 있는 경우, 후속 PR에서 한국어 요약 및 변경 배경을 보강해 이력을 일관화합니다.

## 7) 충돌 대응 표준 절차
- 충돌 탐지: 병합/리베이스 전 `git fetch --all` 후 대상 브랜치와 차이를 먼저 확인합니다.
- 우선순위: 정책/개인정보/인터페이스 계약 문서 변경을 기능 변경보다 우선 반영합니다.
- 해결 원칙: 충돌 파일은 섹션 오너가 최종 정합성을 확인하고 승인합니다.
- 기록 의무: 충돌 원인, 선택한 해법, 후속 예방 조치를 PR 본문에 남깁니다.
- 사후 점검: 충돌 해결 직후 문서 인덱스와 연관 문서 누락 여부를 재검증합니다.


## 8) Main 병합 표준 절차
- 기준 브랜치 이름은 `main`을 표준으로 사용하고, 저장소에 `main`이 없으면 PM/PO가 메인라인 브랜치를 명시합니다(예: `work`).
- 병합 전 절차:
  1. `git fetch --all --prune`
  2. `git checkout <작업브랜치>`
  3. `git merge --no-ff origin/main` 또는 메인라인 브랜치 병합
- 충돌 발생 시 절차:
  1. `git status`로 충돌 파일 식별
  2. 정책/개인정보/계약 문서를 우선 해소
  3. 섹션 오너 승인 후 `git add`
  4. `git commit`으로 충돌 해결 커밋 분리
- 검증 의무:
  - 충돌 마커(`<<<<<<<`, `=======`, `>>>>>>>`) 0건 확인
  - 문서 인덱스(`docs/README.md`) 및 연관 문서 반영 재점검
  - PR 본문에 충돌 원인/해결/재발방지 기록

### 현재 저장소 상태 점검 메모
- 현재 로컬 저장소에는 `origin/main` 추적 정보가 없습니다.
- 따라서 즉시 자동 병합은 불가하며, 원격 연결 후 위 절차로 병합해야 합니다.
- 원격 미연결 상태에서는 내부 충돌 규칙과 문서 정합성 점검을 선반영합니다.
